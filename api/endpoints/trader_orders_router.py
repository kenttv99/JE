...
@router.get("/", response_model=List[schemas.TraderOrderResponse])
...
@router.post("/", response_model=schemas.TraderOrderResponse)
async def create_trader_order(order: schemas.TraderOrderCreate, db: AsyncSession = Depends(get_async_db), current_trader: models.Trader = Depends(get_current_trader)):
    """
    Endpoint to create a new trader order
    """
    return await crud.create_trader_order(db=db, order=order, trader_id=current_trader.id)

@router.put("/{order_id}", response_model=schemas.TraderOrderResponse)
async def update_trader_order(order_id: int, order: schemas.TraderOrderUpdate, db: AsyncSession = Depends(get_async_db)):
    """
...
@router.delete("/{order_id}", response_model=schemas.TraderOrderResponse)
